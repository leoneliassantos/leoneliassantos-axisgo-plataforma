...
import type { Cadastro, Cadastros, NovoProdutoInput, Prioridade } from './data'

export function NovoItem({
  cadastros, opProposta, opPedido, opPrevisao = '', opPrioridade = 'media', opEvento = false, opAmostra = false, opVendedor = '', saving, onAddCadastro, onCreate, onClose,
}: {
  cadastros: Cadastros
  opProposta: string
  opPedido: string
  ...
  function submit() {
    ...
    onCreate(draftToInput(draft, opProposta, opPedido, opPrevisao, opPrioridade, opEvento, opAmostra, opVendedor))
  }
  ...
  return (
    ...
    >
      <ProdutoFields draft={draft} ativos={ativos} opProposta={opProposta} opPedido={opPedido} opPrevisao={opPrevisao} opPrioridade={opPrioridade} opEvento={opEvento} opAmostra={opAmostra} opVendedor={opVendedor} onChange={(patch) => setDraft((d) => ({ ...d, ...patch }))} onAddCadastro={onAddCadastro} />
