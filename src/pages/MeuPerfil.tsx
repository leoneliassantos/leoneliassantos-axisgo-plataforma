...
export function MeuPerfil() {
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-6">
      <div>
        <span className="text-[12px] font-semibold uppercase tracking-[0.14em] text-muted">Conta</span>
      ...
      <form onSubmit={salvar} className="flex flex-col gap-4 rounded-2xl border border-line bg-surface p-6 shadow-card">
        <div className="flex items-center gap-3 border-b border-line pb-4">
          <span className="grid h-11 w-11 place-items-center rounded-full bg-paper font-serif text-lg font-semibold text-ink">
          ...
          <div>
            ...
            <span
              className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${
                user?.role === 'admin' ? 'bg-ink text-white' : 'bg-paper text-ink'
              }`}
            >
        ...
        <div className="flex justify-end">
          <button
            ...
            className="rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-white transition hover:brightness-125 disabled:opacity-50"
          >
