  ...
  return (
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 grid-cols-12 grid-rows-2 gap-2">
        <Tile className="col-span-12 lg:col-span-7" titulo="Evolução do Saldo de Caixa" tip="Trajetória do saldo mês a mês. A linha tracejada marca o zero; pontos vermelhos indicam saldo negativo.">
          <AreaSaldo saldo={m.saldo} labels={m.labels} minIdx={m.minIdx} />
        </Tile>
        <Tile className="col-span-12 lg:col-span-5" titulo="Recebimentos × Pagamentos" tip="Compara, mês a mês, o total de entradas (verde) e saídas (vermelho) de caixa.">
...
function AreaSaldo({ saldo, labels, minIdx }: { saldo: number[]; labels: string[]; minIdx: number }) {
  const W = 860
  const H = 240
  const padL = 6, padR = 6, padT = 20, padB = 22
